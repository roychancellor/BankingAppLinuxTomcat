	...
	private void processCustomerMenu(int option) {
		...
		case 5:
			System.out.println("\nYour minimum monthly payment is "
				+ customers.get(custIndex).getLoan().getMonthlyPayment());
	...
	private void viewEndOfMonth() {
		...
		if(endOfMonth) {
			...
			if(customers.get(custIndex).getSaving().getAccountBalance() < customers.get(custIndex).getSaving().getMinBalance()) {
				System.out.printf("A $%.2f service fee is being assessed for below minimum balance in savings", customers.get(custIndex).getSaving().getServiceFee());
				customers.get(custIndex).getSaving().setAccountBalance(customers.get(custIndex).getSaving().getAccountBalance() - customers.get(custIndex).getSaving().getServiceFee());
			}
			//Compute interest on any positive balance
			if (customers.get(custIndex).getSaving().getAccountBalance() > 0){
				customers.get(custIndex).getSaving().setAccountBalance(customers.get(custIndex).getSaving().getAccountBalance() + (customers.get(custIndex).getSaving().getInterest() * customers.get(custIndex).getSaving().getAccountBalance()));
			}
		}
		else {
